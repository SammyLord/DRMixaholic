...
func main() {
	...
	// Create the salted data for private key generation (POW + usernameSalt)
	saltedPOWAndUser := proofOfWork + usernameSalt
	...
	fmt.Printf("Username Salt Used for Private Key: %s\n", usernameSalt)
	fmt.Println("Proof of Work (base64 of name/project):", proofOfWork)
	fmt.Println("Private Key (salted, sha256(sha512(POW + usernameSalt))):", privateKey)

	fmt.Println("\n--- Instructions ---")
	fmt.Println("1. Your Proof of Work (POW) represents:", powDataString)
	...
	fmt.Println("   To verify, the software will use the POW and the username of the computer it's running on to reconstruct and check the private key.")
	fmt.Println("   The decoded POW (", powDataString, ") will be checked against the author's list.")
